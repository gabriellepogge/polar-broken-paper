...
setwd("/gabriellechristinemartin/Desktop/R_Pol_data_scripts/POL Expt 8")
...
pols1 <- data.frame(read_sav("/Users/gabriellechristinemartin/Desktop/R_Pol_data_scripts/POL Expt 8/POL_Expt8_original.sav"))
...
pols1_abbrev <- pols1 %>%
  dplyr::select(X._task_id, desirepersonal, desireothers, desirebenefit, attdifferent, attsimilar, attoverlap, 
      ...
      mutate(ethnicityomb = as.factor(ethnicityom)) %>%
  ...
  mutate(polar_cond_m2 = as.factor(case_when( 
    ...
    X._task_id ==  "manipinsthighpol" ~ 3,
    X._task_id ==  "manipinstlowpol" ~ 1))) %>% 
  mutate(polar_cond_m12 = as.factor(case_when( 
    X._task_id == "manipinstmed1pol" ~ 2,  
    X._task_id == "manipinstmed2pol" ~ 2,
      X._task_id ==  "manipinsthighpol" ~ 3,
      X._task_id ==  "manipinstlowpol" ~ 1))) %>%
  mutate(polar_cond_m14 = as.factor(case_when( 
    X._task_id ==  "manipinstlowpol" ~ 1,
    X._task_id == "manipinstmed1pol" ~ 2,  
    X._task_id == "manipinstmed2pol" ~ 3,
    X._task_id ==  "manipinsthighpol" ~ 4))) %>%
...
table(pols1_abbrev$polar_cond_m12)
class(pols1_abbrev$polar_cond_m12)

table(pols1_abbrev$X._session_status)
table(pols1_abbrev$complete) #447 completed the session; 349 did not
...
levels(pols1_abbrev$polar_cond_m12) <- c("Low Polarization", "Medium1 & Medium2 Polarization","High Polarization")
summary(pols1_abbrev$polar_cond_m12)

levels(pols1_abbrev$polar_cond_m14) <- c("Low Polarization", "Medium1 Polarization", "Medium2 Polarization","High Polarization")
summary(pols1_abbrev$polar_cond_m14)
...
#n, complete_DV = 491 (dropped n = 305)
#n, complete = 447
#droped Ps under 18 (n = 0): results in 491 responses

pols1_abbrev2 <- pols1_abbrev %>%
  dplyr::select(desirepersonal, desireothers, desirebenefit, attdifferent, attsimilar, attoverlap, 
         brokensystem, systempoorfunct, systemflawed, systemfailing,
         trust1, trust2, trust3, trust4, trust5, exptdemand1, exptdemand2, exptdemand3, exptdemand4, exptdemand5,
         polorient, partyid, age, gender, education, ethnicityomb, raceomb, polar_cond_m1, polar_cond_m2, 
         polar_cond_m12, polar_cond_m14,complete) %>%
...
#get the reliability for the three manipulation check items: alpha = .84
...
#get the reliability for the three primary DV items: alpha = .82
...
#get the reliability for the nine mediator items: alpha = .91
...
amodel_omnibus_demand3_m14 <- aov(exptdemand3 ~ polar_cond_m14, data = pols1_abbrev2)
summary(amodel_omnibus_demand3_m14)

gh.demand3_m1m4 <- oneway(pols1_abbrev2$polar_cond_m14, y = pols1_abbrev2$exptdemand3, posthoc = 'tukey')
gh.demand3_m1m4
...
# test differences between medium conditions

pols1_abbrev2 <- pols1_abbrev2 %>%
mutate(M1vM2 = factor(case_when(
  polar_cond_m1 == "Medium1 Polarization" ~ "Medium1 Polarization",
  polar_cond_m2 == "Medium2 Polarization" ~ "Medium2 Polarization")))

polarcond_polarcheck_m1m2 <- pols1_abbrev2 %>%
  select(polar_check, M1vM2) %>%
  filter(M1vM2 != "NA" & polar_check > -99)

gh.manip_check_scale_m1m2 <- oneway(polarcond_polarcheck_m1m2$M1vM2, y = polarcond_polarcheck_m1m2$polar_check, posthoc = 'games-howell')
gh.manip_check_scale_m1m2

cohensD(x = polar_check ~ M1vM2, data = polarcond_polarcheck_m1m2, method = "unequal")
...
polarcond_desire_m12 <- pols1_abbrev2 %>%
  select(desire_scale, polar_cond_m12) %>%
  filter(polar_cond_m12 != "NA" & desire_scale > -99)

# polarcond_desire_m1, n = 410
with(polarcond_desire_m1, lawstat::levene.test(desire_scale, as.factor(polar_cond_m1),location="mean"))

# polarcond_desire_m1, n = 407
with(polarcond_desire_m2, lawstat::levene.test(desire_scale, as.factor(polar_cond_m2),location="mean"))

# polarcond_desire_m1, n = 491
with(polarcond_desire_m12, lawstat::levene.test(desire_scale, as.factor(polar_cond_m12),location="mean"))

# both medium conditions show equal vairances 
...
amodel_omnibus_desire_m2 <- aov(desire_scale ~ polar_cond_m2, data = pols1_abbrev2)
summary(amodel_omnibus_desire_m2)

amodel_omnibus_desire_m12 <- aov(desire_scale ~ polar_cond_m12, data = pols1_abbrev2)
summary(amodel_omnibus_desire_m12)
...
summary.aov(amodel_omnibus_desire_m2, split=list(polar_cond_m2=list("High vs. Low"=2, 
                                                                    "High & Low vs Medium" = 1))) 
...
sqrt(.052)/sqrt(.052 + 407)

# High and low versus medium2 contrast

sqrt(4.292)/sqrt(4.292 + 404)

# High versus low contrast

sqrt(4.757)/sqrt(4.757 + 407)


# assessing the linear effect (rather than contrats) with Tukey pairwise comparisons for Medium1 and Medium2

tk.desire_scale_m1 <- oneway(polarcond_desire_m1$polar_cond_m1, y = polarcond_desire_m1$desire_scale, posthoc = 'tukey')
tk.desire_scale_m1

gh.desire_scale_m2 <- oneway(polarcond_desire_m2$polar_cond_m2, y = polarcond_desire_m2$desire_scale, posthoc = 'tukey')
gh.desire_scale_m2
...
polarcond_desire_m1m2 <- pols1_abbrev2 %>%
  select(desire_scale, M1vM2) %>%
  filter(M1vM2 != "NA" & desire_scale > -99)

gh.desire_scale_m1m2 <- oneway(polarcond_desire_m1m2$M1vM2, y = polarcond_desire_m1m2$desire_scale, posthoc = 'tukey')
gh.desire_scale_m1m2

cohensD(x = desire_scale ~ M1vM2, data = polarcond_desire_m1m2, method = "equal")
...
polarcond_broken_m1 <- pols1_abbrev2 %>%
  dplyr::select(brokenness_scale, polar_cond_m1) %>%
...
sqrt(1.347)/sqrt(1.347 + 390)

# High versus low contrast

sqrt(0.096)/sqrt(0.096 + 390)


# dealing with the heterogeneity of variance for mediumm1 condition

# Welch's test
wmodel_omnibus_polarcheck_m1 <- oneway.test(brokenness_scale ~ as.factor(polar_cond_m1), data = polarcond_broken_m1)
wmodel_omnibus_polarcheck_m1
...
pols1_abbrev2 %>%
  dplyr::select(polar_cond_m1, polar_cond_m2, brokenness_scale) %>%
...
SimTestDiff(data = polarcond_broken_m1, grp="polar_cond_m1", resp="brokenness_scale", ContrastMat = mat2,
            covar.equal = FALSE)



# assessing the linear effect with Tukey pairwise comparisons for Medium2 and Games-Howell for Medium1

tk.brokenness_scale_m1 <- oneway(polarcond_broken_m1$polar_cond_m1, y = polarcond_broken_m1$brokenness_scale, posthoc = 'games-howell')
...
#first, check the linear effect on desire. Negative linear effects here in both cases.
...
#first, check the linear effect on brokenness No effect, as expected.
