...
setwd("/gabriellechristinemartin/Desktop/R_Pol_data_scripts/POL Expt 11")
...
pols1 <- data.frame(read_sav("/Users/gabriellechristinemartin/Desktop/R_Pol_data_scripts/POL Expt 11/POL_Expt11_original.sav"))
...
pols1_abbrev <- pols1 %>%
  dplyr::select(polarcond, brokencond, attsimilar, attdifferent, attoverlap,
         ...
         polorient, partyid, age, gender, ethnicityom, raceomb, finished) %>%
  ...
  mutate(finished = as.factor(case_when(
    finished == "yes" ~ 1)))
...
levels(pols1_abbrev$finished) <- c("P reached end of study")
summary(pols1_abbrev$finished)


# this is what we will want to report in the manuscript: complete_DV & age > 18 (ignoring whether Ps got to the end)
# n, complete/finished study = 1343

# starting n = 1490
# dropped 76 incomplete DV (all three DV responses not present), n = 1414
#droped Ps under 18 (n = 451): results in 963 responses
# final n = 963
...
pols1_abbrev2 <- pols1_abbrev %>%
  select(polar_cond, broken_cond, attsimilar, attdifferent, attoverlap,
         ...
         fixsystem6, fixsystem7, fixsystem8, fixsystem9, fixsystem10,
         exptdemand1, exptdemand2, exptdemand3, exptdemand4, exptdemand5,
         polorient, partyid, age, gender, ethnicityom, raceomb) %>%
...
#3.1a. Reliability for the three polarization manipulation check items: alpha = .87
...
#3.2a. Reliability for the four brokenness manipulation check items: alpha = .96
...
#3.3a. Reliability for the five dis(trust) items: alpha = .89
# no need to reverse-code
...
#3.4a. Reliability for the nine brokenness + dis(trust) items: alpha = .94
...
#3.5a. Reliability for the three primary DV items: alpha = .83
...
#3.6a. Reliability for the 8 secondary support for third parties items: alpha = .91
...
#3.7a. Reliability for the 10 general fixes items: alpha = .86
...
#3.8a. Reliability for the 5 experimenter demand items: alpha = .36
...
pols1_abbrev2 <- pols1_abbrev2 %>%
  mutate(exptdemand_scale =
           (exptdemand1 + exptdemand2 + exptdemand3 +
            exptdemand4 + exptdemand5)/5)
...
# we have unequal variances across levels 
...
#5. ANALYSES OF THE DVs IN ANOVA WITH CONTRASTS (AS IN THE MANUSCRIPT) ----
...
sqrt(18.985)/sqrt(18.985 + 956)

# High and low versus medium contrast

sqrt(14.494)/sqrt(14.494 + 956)

# High versus low contrast

sqrt(2.920)/sqrt(2.920 + 956)

# interaction between brokenness condtiion and low versus high (excluding medium) polarization condition

sqrt(12.967)/sqrt(12.967 + 956)
...
# 5.3b. DV = general support for third parties and candidates

# check for homogeneity of variance across the 6 conditions for the secondary DV: thirdsupport_scale
...
# unequal variances so will need to report adjusted contrasts
# third party support DV omnibus test
amodel_omnibus_support <- aov(thirdsupport_scale ~ brokenpol_cond_fact, data = pols1_abbrev2)

# Welch's omnibus test for unequal variance
wmodel_omnibus_support <- oneway.test(thirdsupport_scale ~ as.factor(brokenpol_cond_fact), data = polarbroken_support,
                                         var.equal = FALSE)
wmodel_omnibus_support
...
pols1_abbrev2 %>%
  dplyr::select(polar_cond, broken_cond, brokenpol_cond_fact, thirdsupport_scale) %>%
  write.csv("/Users/gabriellechristinemartin/Desktop/R_Pol_data_scripts/S4ab_adjusted_contrasts/S4b_adjusted_contrasts/S4b_simcomp.csv", row.names = TRUE)

# testing the separate/isolated effect of polarization condition on support for third parties
amodel_omnibus_supportb <- aov(thirdsupport_scale ~ polar_cond, data = pols1_abbrev2)
summary.aov(amodel_omnibus_supportb, split=list(polar_cond=list("Code 2: contrast_lhvm" = 1, "Code 3: contrast_lvh" = 2)))
...
# 5.3c. DV =  support for alternative methods of fixing a broken electoral system

# check for homogeneity of variance across the 6 conditions for the secondary DV: genfixes_scale
...
# desire DV omnibus test
amodel_omnibus_fixes <- aov(genfixes_scale ~ brokenpol_cond_fact, data = pols1_abbrev2)
summary(amodel_omnibus_fixes)

# the below provides the estimates for the two contrasts and the omnibus model
# Make sure to use summary.aov here or 'split' might not work
# make sure the contrasts are listed in the same order they appear in the matrix!
summary.aov(amodel_omnibus_fixes, split=list(brokenpol_cond_fact=list("Code 1: contrast_bvnb" = 1,"Code 2: contrast_lhvm" = 2, 
                                                                       "Code 3: contrast_lvh" = 3, "Code 4: contrast_blhvm" = 4,
                                                                       "Code 5: contrast_blvh" = 5)))
...
sqrt(8.089)/sqrt(8.089 + 941)

# High and low versus medium contrast

sqrt(0.691)/sqrt(0.691 + 941)

# High versus low contrast

sqrt(0.008)/sqrt(0.008 + 941)


# testing the separate/isolated effect of polarization condition on desire
amodel_omnibus_fixesb <- aov(genfixes_scale ~ polar_cond, data = pols1_abbrev2)
summary.aov(amodel_omnibus_fixesb, split=list(polar_cond=list("Code 2: contrast_lhvm" = 1, "Code 3: contrast_lvh" = 2)))
...
#6. Testing differences between all cell means in the table
...
support_bypolarcond_B1 <- oneway(B1$polar_cond, y = B1$thirdsupport_scale, posthoc = 'tukey')
support_bypolarcond_B1

genfixes_bypolarcond_B1 <- oneway(B1$polar_cond, y = B1$genfixes_scale, posthoc = 'tukey')
genfixes_bypolarcond_B1
...
support_bypolarcond_B2 <- oneway(B2$polar_cond, y = B2$thirdsupport_scale, posthoc = 'tukey')
support_bypolarcond_B2

genfixes_bypolarcond_B2 <- oneway(B2$polar_cond, y = B2$genfixes_scale, posthoc = 'tukey')
genfixes_bypolarcond_B2
...
#6.4a. model with the 5 contrast codes predicting thirdsupport_scale
rmodel_support <- lm(thirdsupport_scale ~ code1+code2+code3+code4+code5, data = pols1_abbrev2)
summary(rmodel_support)

#6.5a. model with the 5 contrast codes predicting genfixes_scale
rmodel_fixes <- lm(genfixes_scale ~ code1+code2+code3+code4+code5, data = pols1_abbrev2)
summary(rmodel_fixes)


# ADD DEMAND AS A COVARIATE IN REGRESSION: may have to mess with these some because of the crappy alpha
...
#6.4b. model with the 5 contrast codes predicting thirdsupport_scale
rmodel_support_b <- lm(thirdsupport_scale ~ code1+code2+code3+code4+code5+exptdemand_scale, data = pols1_abbrev2)
summary(rmodel_support_b)
lm.beta(rmodel_support_b)

#6.5b. model with the 5 contrast codes predicting genfixes_scale
rmodel_fixes_b <- lm(genfixes_scale ~ code1+code2+code3+code4+code5+exptdemand_scale, data = pols1_abbrev2)
summary(rmodel_fixes_b)
lm.beta(rmodel_fixes_b)
...
#6.4c. model with the 6 main effects (contrast codes predicting general support for third parties) and 5 interaction terms

rmodel_support_c <- lm(thirdsupport_scale ~ code1+code2+code3+code4+code5+demand_C+
                        code1*demand_C+code2*demand_C+code3*demand_C+
                        code4*demand_C+code5*demand_C, data = pols1_abbrev2)
summary(rmodel_support_c)
lm.beta(rmodel_support_c)

# this method does not first center the main effect and I assume the multicollinearity
# washes out the effect of brokenness condition
rmodel_support_d <- lm(thirdsupport_scale ~ code1+code2+code3+code4+code5+exptdemand_scale+
                        code1*exptdemand_scale+code2*exptdemand_scale+code3*exptdemand_scale+
                        code4*exptdemand_scale+code5*exptdemand_scale, data = pols1_abbrev2)
summary(rmodel_desire_d)
lm.beta(rmodel_desire_d)
...
#6.5c. model with the 6 main effects (contrast codes predicting support for alternative methods of fixing brokenness) and 5 interaction terms

rmodel_fixes_c <- lm(genfixes_scale ~ code1+code2+code3+code4+code5+demand_C+
                         code1*demand_C+code2*demand_C+code3*demand_C+
                         code4*demand_C+code5*demand_C, data = pols1_abbrev2)
summary(rmodel_fixes_c)
lm.beta(rmodel_fixes_c)

# this method does not first center the main effect and I assume the multicollinearity
# washes out the effect of brokenness condition
rmodel_fixes_d <- lm(genfixes_scale ~ code1+code2+code3+code4+code5+exptdemand_scale+
                         code1*exptdemand_scale+code2*exptdemand_scale+code3*exptdemand_scale+
                         code4*exptdemand_scale+code5*exptdemand_scale, data = pols1_abbrev2)
summary(rmodel_fixes_d)
lm.beta(rmodel_fixes_d)
